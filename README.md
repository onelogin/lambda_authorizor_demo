# AWS API Gateway Custom Authorizer for Onelogin JWTs
		...
		"Statement": [
			{
				...
				"Effect": "Allow",
				"Resource": "arn:aws:execute-api:us-east-1:<AWSAccountID>:<AWSApiGatewayID>/dev/*/pets/"
			}
		]
	}
}
...
This will generate a local `onelogin-authorizer.zip` bundle (ZIP file) containing all the source, configuration and node modules an AWS Lambda needs.
...
4. Provide the role a name. Eg: `OneLogin-Authorizer-Role`
...
* Name: `OneLoginCustomAuthorizer`
* Description: `One Login Custom Authorizer`
* Runtime: `Node.js 12.x`
* _Lambda function code_
    * Code entry type: `Update a .ZIP file`
    * Function package: (upload the `onelogin-authorizer.zip` file created earlier)
...
* _Lambda function handler and role_
    ...
    * Existing role: `OneLogin-Authorizer-Role` (same role created earlier)
* _Advanced Settings_
    * Timeout: `30` seconds
...
4. Use the following values in the **New Custom Authorizer** form:
   ...
   * Lambda function: `OneLoginCustomAuthorizer`
   * Authorizer name: `onelogin-custom-authorizer`
...
You can then test the new custom authorizer by providing an **Access Token** and clicking **Test**. The ACCESS_TOKEN is the same format we used in `event.json` above:
...
4. Under the **Settings** section, click the pencil to the right of the **Authorization** and choose the `onelogin-custom-authorizer` Custom Authorizer. Click the checkbox to the right of the drop down to save.
...
OneLogin helps you to:

* Add authentication with [multiple authentication sources](https://), either social like **Google, Facebook, Microsoft Account, LinkedIn, GitHub, Twitter, Box, Salesforce, amongst others**, or enterprise identity systems like **Windows Azure AD, Google Apps, Active Directory, ADFS or any SAML Identity Provider**.
* Add authentication through more traditional **username/password**.
* Analytics of how, when and where users are logging in.


## Create a free account in OneLogin

1. Go to [OneLogin](https://www.onelogin.com) and click Sign Up.

## Issue Reporting

If you have found a bug or if you have a feature request, please report them at this repository issues section. Please do not report security vulnerabilities on the public GitHub issue tracker. The [Responsible Disclosure Program](https://onelogin.com/security) details the procedure for disclosing security issues.
